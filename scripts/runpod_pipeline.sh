...
for value in RUNPOD_CONTAINER_DISK_GB RUNPOD_VOLUME_GB RUNPOD_MAX_HOURS RUNPOD_TERMINATE_AFTER_HOURS; do
...
log "Running the documented pipeline on the clean Pod"
ssh "${SSH_ARGS[@]}" "root@${SSH_IP}" \
  "cd ${QUOTED_ROOT} && make pipeline PROFILE=${QUOTED_PROFILE}" \
  2>&1 | tee "${LOCAL_LOG_DIR}/${POD_ID}.log"
